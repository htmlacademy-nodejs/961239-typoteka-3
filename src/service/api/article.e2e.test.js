...
describe(`Create new article with correct data`, () => {
  ...
  test(`Article has correct title`, () => expect(listResponse.body[5].title).toEqual(`Тестовый заголовок статьи`));
  test(`Article has correct announce`, () => expect(listResponse.body[5].announce).toEqual(`Тестовый анонс`));
  test(`Article has correct fullText`, () => expect(listResponse.body[5].fullText).toEqual(`Тестовый полный текст.`));
  test(`Article has correct categories`, () => expect(listResponse.body[5].categories).toEqual([`Тестовая категория`]));
});
...
describe(`Edit article with correct data`, () => {
  ...
  test(`Article has correct categories`, async () => expect(articleDataResponse.body.categories).toEqual([`IT`, `Деревья`]));
...
describe(`Return comments list`, () => {
  ...
  test(`Status code 200`, () => expect(response.statusCode).toBe(parseInt(StatusCode.OK, 10)));
  test(`Get comments`, () => expect(response.body).toEqual(mockData[mockData.findIndex((elem) => elem.id === 3)].comments));
});
